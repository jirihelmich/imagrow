...
<body>
  ...
  <div class="container">
    ...
    <p>When you launch ImaGrow for the first time, you are presented with the login screen. Since the application stores data locally, your account exists only on your machine — it is not shared with anyone.</p>
    ...
    <p>The core purpose of ImaGrow is to track how a premature child grows relative to reference data. This is done by recording examinations at each clinical visit and reviewing the resulting charts and statistics.</p>
    ...
    <p>On the dashboard, the <strong>Export</strong> button (top right) downloads a single <code>imagrow-export.json</code> file containing your complete database — patients, examinations, and parent data. Save it somewhere outside the app's storage: a shared drive, OneDrive, or an external disk. A weekly cadence is a reasonable default; do the export before any larger change (import of many patients, upgrade of the app).</p>
    ...
    <ul>
      <li><strong>Windows:</strong> <code>C:\\Users\\&lt;user&gt;\\AppData\\Roaming\\ImaGrow\\IndexedDB\\</code></li>
    ...
    <p>Installing a newer release does <strong>not</strong> touch the database. The installer replaces only the application binaries in <code>/Applications/</code> (macOS) or <code>Program Files\\ImaGrow\\</code> (Windows); your data in the profile folder above stays intact. The new version reads the existing IndexedDB, checks the stored schema version, and runs a migration only if the structure changed between releases.</p>

    <p>Recommended upgrade routine:</p>

    <ol>
      <li>Open ImaGrow and click <strong>Export</strong> on the dashboard; save the JSON somewhere safe.</li>
      <li>Close ImaGrow.</li>
    ...
    <p>During the <strong>uninstall</strong> of a specific version, the installer leaves the data folder in place by default. The database is only deleted by manually removing <code>~/Library/Application Support/ImaGrow/</code> (macOS) or <code>%APPDATA%\\ImaGrow\\</code> (Windows), or by ticking a "remove user data" option if one is presented during uninstall.</p>
...
function generateCzechMarkdown(cs, en) {
  ...
  const md = `# ImaGrow — Uživatelská příručka
...
- **Windows:** \`C:\\Users\\<uživatel>\\AppData\\Roaming\\ImaGrow\\IndexedDB\\\`
- **macOS:** \`~/Library/Application Support/ImaGrow/IndexedDB/\`
...
<body>
  ...
  <div class="container">
    ...
    <p>Hlavním účelem aplikace ImaGrow je sledovat, jak předčasně narozené dítě roste ve srovnání s referenčními daty.</p>
    ...
    <p>Na úvodní obrazovce je vpravo nahoře tlačítko <strong>Export</strong>. Klepnutím se stáhne soubor <code>imagrow-export.json</code> s kompletní databází — pacienti, vyšetření, údaje o rodičích. Uložte ho někam mimo aplikaci: sdílený disk, OneDrive, externí disk. Týdenní rytmus je rozumné minimum; export určitě udělejte před větší změnou (import více pacientů, aktualizace aplikace).</p>
    ...
    <ul>
      <li><strong>Windows:</strong> <code>C:\\Users\\&lt;uživatel&gt;\\AppData\\Roaming\\ImaGrow\\IndexedDB\\</code></li>
      <li><strong>macOS:</strong> <code>~/Library/Application Support/ImaGrow/IndexedDB/</code></li>
    </ul>
    ...
    <p>Při <strong>odinstalaci</strong> konkrétní verze instalátor defaultně datovou složku ponechá. Databáze se smaže až ručním odstraněním <code>~/Library/Application Support/ImaGrow/</code> (macOS) nebo <code>%APPDATA%\\ImaGrow\\</code> (Windows), případně zaškrtnutím volby „odstranit uživatelská data", pokud je při odinstalaci nabídnuta.</p>
